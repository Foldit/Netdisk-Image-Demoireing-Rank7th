...
class Rotate:
     ...
     def __call__(self, im1, im2):
        if random.random() < self.prob:
            dushu = random.uniform(0, 90)
            im1 = rotate(im1,dushu)
            im2 = rotate(im2,dushu)
        return im1, im2
...
def resize(im, target_size=608, interp=cv2.INTER_AREA):
...
def croph(im,i,j,h,w):
   
    return F.crop(im, i, j, h, w)


class Crop:
    def __init__(self, target=(512, 512),prob=0.5):
        self.prob = prob
        self.target = target

    def __call__(self, im1, im2):
        width, height= im1.shape[0],im1.shape[1]
        w = self.target[0]
        h = self.target[1]
        i = random.randint(0, height - h)
        j = random.randint(0, width - w)
        if random.random() < self.prob:
            im1 = croph(im1, i,j,h+i,w+j)
            im2 = croph(im2, i,j,h+i,w+j)
        return im1, im2
